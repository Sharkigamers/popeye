...
def get_redis():
    if not hasattr(g, 'redis'):
        g.redis = Redis(host=os.getenv("CONNECT_ENV"), db=0, socket_timeout=5)
    return g.redis
...
if __name__ == "__main__":
    try:
        if (os.getenv("FLASK_RUN_HOST")):
            myHost = os.getenv("FLASK_RUN_HOST")
    except:
        myHost = '0.0.0.0'
    try:
        if (os.getenv("FLASK_RUN_PORT")):
            myPort = int(os.getenv("FLASK_RUN_PORT"))
    except:
        myPort = 80
    app.run(host=myHost, port=myPort, debug=True, threaded=True)
